...
def process(audio, sr, model, args):
    ...
    if args["updates"]:
        ...
        for seg in audio:
            if seg.shape[0] < n_wins:
                ...
                seg = to_pad
            with torch.no_grad():
                out = model(seg.unsqueeze(0).float(), torch.as_tensor(n_wins).unsqueeze(0))
            print(out[0].numpy())

    else:
        with torch.no_grad():
            out = model(audio.unsqueeze(0).float(), torch.as_tensor(n_wins).unsqueeze(0))
        print(out[0].numpy())
    return out
